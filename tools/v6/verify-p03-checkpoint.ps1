...
$checkpointPath = Join-Path $checkpointRoot 'SPD_GC_V6_P03_R1_RUNTIME_CHECKPOINT.zip'
...
& 'D:\Environment\PowerShell\7\pwsh.exe' -NoProfile -File (Join-Path $repoRoot 'tools/v6/create-p00-checkpoint.ps1') `
...
& 'D:\Environment\PowerShell\7\pwsh.exe' -NoProfile -File (Join-Path $unpackedRoot 'tools/v6/verify-p03-source-tree.ps1') `
...
& 'D:\Environment\PowerShell\7\pwsh.exe' -NoProfile -File (Join-Path $unpackedRoot 'tools/v6/run-p03-gates.ps1') `
...
$gateResults = Get-Content -LiteralPath (Join-Path $gateOutput 'P03_GATE_RESULTS.json') -Raw | ConvertFrom-Json
$status = if ($gateExitCode -eq 0 -and $gateResults.status -eq 'PASS') { 'PASS' } else { 'FAIL' }

@(
    '# P03-R1 Checkpoint 解压副本复验报告'
    ''
    "- Status: $status"
    '- Checkpoint: SPD_GC_V6_P03_R1_RUNTIME_CHECKPOINT.zip'
    ...
    '- Unpacked gate results: checkpoint-verification/gates/P03_GATE_RESULTS.json'
...
Write-Output "P03_CHECKPOINT_VERIFY_PASS sha256=$checkpointHash files=$verifiedFiles"
