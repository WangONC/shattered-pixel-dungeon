...
$env:P03_ARTIFACT_DIR = $artifactRoot
...
Invoke-RecordedGate 'p03-source-tree' $pwsh @(
    '-NoProfile', '-File', (Join-Path $repoRoot 'tools/v6/verify-p03-source-tree.ps1'), '-Root', $repoRoot
)
Invoke-RecordedGate 'p03-architecture-guard' $gradle @(
    ...
    '--tests', 'com.shatteredpixel.shatteredpixeldungeon.rules.contract.v6.P03TypedSkillArchitectureTest'
)
Invoke-RecordedGate 'p03-layer-a-schema' $gradle @(
    '--console=plain', '--rerun-tasks', ':core:test',
    '--tests', 'com.shatteredpixel.shatteredpixeldungeon.rules.contract.v6.P03SkillFormSchemaTest'
)
Invoke-RecordedGate 'p03-layer-b-builder-compile' $gradle @(
    '--console=plain', '--rerun-tasks', ':core:test',
    '--tests', 'com.shatteredpixel.shatteredpixeldungeon.rules.contract.v6.P03PlayerBuilderVerticalSliceTest'
)
Invoke-RecordedGate 'p03-layer-c-validation-formatter-budget' $gradle @(
    '--console=plain', '--rerun-tasks', ':core:test',
    '--tests', 'com.shatteredpixel.shatteredpixeldungeon.rules.contract.v6.P03SkillValidationAndCatalogTest'
)
Invoke-RecordedGate 'p03-layer-d-runtime' $gradle @(
    '--console=plain', '--rerun-tasks', ':core:test',
    '--tests', 'com.shatteredpixel.shatteredpixeldungeon.rules.contract.v6.P03DirectDamageRuntimeBehaviorTest'
)
Invoke-RecordedGate 'p03-layer-e-save-load' $gradle @(
    '--console=plain', '--rerun-tasks', ':core:test',
...
Invoke-RecordedGate 'p00-p03-contract-tests' $gradle @(
    '--console=plain', '--rerun-tasks', ':core:test',
...
$result | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath (Join-Path $deliveryRoot 'P03_GATE_RESULTS.json') -Encoding utf8NoBOM

if ($failed.Count -gt 0) {
    Write-Error ('P03 gates failed: ' + ($failed -join ', '))
    exit 1
}
Write-Output 'P03_GATES_PASS'
