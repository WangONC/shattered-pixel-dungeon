...
& 'D:\Environment\PowerShell\7\pwsh.exe' -NoProfile -File (Join-Path $unpackedRoot 'tools/v6/verify-p02-source-tree.ps1') `
...
& 'D:\Environment\PowerShell\7\pwsh.exe' -NoProfile -File (Join-Path $unpackedRoot 'tools/v6/run-p02-gates.ps1') `
...
$gateResults = Get-Content -LiteralPath (Join-Path $gateOutput 'P02_GATE_RESULTS.json') -Raw | ConvertFrom-Json
...
Write-Output "P02_CHECKPOINT_VERIFY_PASS sha256=$checkpointHash files=$verifiedFiles"
