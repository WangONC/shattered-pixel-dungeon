...
$javaHome = 'C:\User\Environment\Java\jdk-24'
$gradle = Join-Path $repoRoot 'gradlew.bat'
$pwsh = 'C:\Program Files\PowerShell\7\pwsh.exe'
$env:JAVA_HOME = $javaHome
$env:ANDROID_HOME = 'C:\User\Application\Android\SDK'
$env:ANDROID_SDK_ROOT = 'C:\User\Application\Android\SDK'
$env:P01_ARTIFACT_DIR = $artifactRoot
...
Invoke-RecordedGate 'p00-source-tree' $pwsh @(
    '-NoProfile', '-File', (Join-Path $repoRoot 'tools/v6/verify-p00-source-tree.ps1'), '-Root', $repoRoot
)
Invoke-RecordedGate 'p00-architecture-guard' $gradle @(
...
Invoke-RecordedGate 'p01-contract-tests' $gradle @(
    '--console=plain', '--rerun-tasks', ':core:test',
...
$result = [ordered]@{
    phase = 'P01'
    baseline = '1f3f6678eb7b8fa43df18deb5a46eda7e601a4b9'
    generated_at = [DateTimeOffset]::Now.ToString('o')
    java_home = $javaHome
    android_sdk = 'C:\User\Application\Android\SDK'
    status = if ($failed.Count -eq 0) { 'PASS' } else { 'FAIL' }
    failed = @($failed)
    gates = $records
}
$result | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath (Join-Path $deliveryRoot 'P01_GATE_RESULTS.json') -Encoding utf8NoBOM

if ($failed.Count -gt 0) {
    Write-Error ('P01 gates failed: ' + ($failed -join ', '))
    exit 1
}
Write-Output 'P01_GATES_PASS'
